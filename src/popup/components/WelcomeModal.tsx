...
export default function WelcomeModal({ settings, onComplete, onOpenSettings }: WelcomeModalProps) {
  ...
  const apiKeyValidation = useMemo(
    () => validateApiKeyFormat(apiKey, selectedProvider),
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <div className="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center flex-shrink-0">
                  <svg aria-hidden="true" className="w-4 h-4 text-green-600 dark:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <div className="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center flex-shrink-0">
                  <svg aria-hidden="true" className="w-4 h-4 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {step === 'quick-setup' && (
          <div className="p-6">
            ...
            {/* 服务商选择 */}
            <div className="space-y-2 mb-4" role="radiogroup" aria-label="选择翻译服务商">
              {QUICK_PROVIDERS.map(provider => (
                ...
                >
                  ...
                  {selectedProvider === provider.id && (
                    <svg aria-hidden="true" className="w-5 h-5 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-3 py-2 pr-10 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent ${
                    apiKeyValidation.type === 'error'
                      ? 'border-red-400 dark:border-red-500'
                      : apiKeyValidation.type === 'success' && apiKey.trim().length > 0
              ...
              {/* API Key 格式验证提示 */}
              {apiKey.trim().length > 0 && testResult === null && (
                <p role="status" className={`mt-1 text-xs ${
                  apiKeyValidation.type === 'error' ? 'text-red-500' : 'text-green-500'
                }`}>
                  {apiKeyValidation.message}
                </p>
              )}
              {testResult === 'error' && (
                <p role="status" className="mt-1 text-xs text-red-500">连接测试失败，请检查 API Key 或网络</p>
              )}
              {testResult === 'success' && (
                <p role="status" className="mt-1 text-xs text-green-500">连接测试成功！</p>
              )}
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={isTesting || !apiKey.trim() || apiKeyValidation.type === 'error'}
                className="flex-1 py-2 px-4 bg-primary-600 hover:bg-primary-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
              >
                {isTesting ? (
                  <>
